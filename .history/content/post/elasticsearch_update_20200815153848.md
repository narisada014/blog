...
{{< highlight html >}}
{
                                query: {
                                    bool: {
                                        must: queries.map do |q|
                                          {bool: {
                                              should: [
                                                  {match: {item_name: q}},
                                                  {match: {brand_name: {query: q, boost: 5}}},
                                                  {match: {maker_name: {query: q, boost: 5}}},
                                                  {match: {tag_name: {query: q, boost: 10}}},
                                                  {match: {category_name: {query: q, boost: 8}}},
                                                  {match: {keywords: q}},
                                              ]
                                              should: self.match_query(q)
                                          }}
                                        end
                                    }
                                    # multi_match: {
                                    #     query: query,
                                    #     fields: fields,
                                    #     minimum_should_match: query.split(' ').count
                                    # }
                                }
                            }
{{< /highlight >}}
